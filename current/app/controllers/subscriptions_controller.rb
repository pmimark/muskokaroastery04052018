class SubscriptionsController < ApplicationController
  ...
  #     @cancelled_subscriptions =  current_user.subscriptions.where(status: "Cancel").page(params[:page]).per(20)
  #   end 
  # end
  ...
  def show
    ...
    if @subscription.order.billing_same_as_shipping
      shipping_country = @subscription.order.country
    else
  ...
  def update
    ...
    respond_to do |format|
      if @order.update(order_params)
        format.html { redirect_to("/subscriptions/#{@subscription.id}", :notice => 'Order was successfully updated.') }
        format.xml  { head :ok }
      else
